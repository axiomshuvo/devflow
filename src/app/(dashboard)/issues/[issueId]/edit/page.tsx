...
const ISSUE_TYPES: { value: IssueType; label: string; icon: React.ReactNode; color: string }[] = [
  ...
  { value: "FEATURE",     label: "Feature",     icon: <MdRocketLaunch />, color: "text-purple-500" },
  { value: "IMPROVEMENT", label: "Improvement", icon: <MdTrendingUp />,   color: "text-green-500" },
];

const PRIORITIES: { value: IssuePriority; label: string; icon: React.ReactNode; color: string }[] = [
  { value: "LOW",      label: "Low",      icon: <MdArrowDownward />,                                color: "text-gray-400" },
  { value: "MEDIUM",   label: "Medium",   icon: <MdArrowForward />,                                 color: "text-amber-500" },
  { value: "HIGH",     label: "High",     icon: <MdArrowUpward />,                                  color: "text-orange-500" },
  { value: "CRITICAL", label: "Critical", icon: <MdKeyboardDoubleArrowDown className="rotate-180" />, color: "text-red-600" },
];
...
const MILESTONES    = ["v1.0 Launch", "v1.1 Hotfix", "v2.0 Major Release", "Q2 Sprint", "Sprint 2"];
const ENVIRONMENTS  = ["Production", "Staging", "Development", "QA"];
const VERSIONS      = ["v1.0.0", "v1.1.0", "v1.2.0", "v2.0.0-beta"];
const COMPONENTS    = ["Authentication", "Dashboard", "API Gateway", "Database", "Frontend", "Mobile"];
...
const MOCK_ATTACHMENTS: Record<string, { name: string; size: string; date: string; type: "image" | "file" }[]> = {
  i1: [
    { name: "screenshot-login-bug.png", size: "221 KB", date: "May 18, 2024", type: "image" },
    { name: "console-error-log.txt",    size: "12 KB",  date: "May 18, 2024", type: "file" },
  ],
};
...
export default function EditIssuePage({ params }: { params: Promise<{ issueId: string }> }) {
  ...
  const [projectId,    setProjectId]    = useState(issue?.projectId ?? mockProjects[0].id);
  ...
  const [selectedLabels, setSelectedLabels] = useState<string[]>(
    issue?.labels?.map((l) => ALL_LABELS.find((x) => x.name === l.name)?.id ?? l.id) ?? []
  );
  const [milestone,    setMilestone]    = useState("");
  const [dueDate,      setDueDate]      = useState(
    issue?.dueDate ? new Date(issue.dueDate).toISOString().split("T")[0] : ""
  );
  const [status,       setStatus]       = useState<"BACKLOG" | "TODO" | "IN_PROGRESS" | "IN_REVIEW" | "DONE" | "BLOCKED">(issue?.status ?? "TODO");
  const [environment,  setEnvironment]  = useState("");
  const [version,      setVersion]      = useState("");
  const [component,    setComponent]    = useState("");
  const [dragOver,     setDragOver]     = useState(false);
  const [newFiles,     setNewFiles]     = useState<string[]>([]);
  const [existingFiles, setExistingFiles] = useState(MOCK_ATTACHMENTS[issueId] ?? []);
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="px-6 pt-6 pb-0">
        ...
        <div className="flex items-center gap-1.5 text-sm text-gray-500 mb-3">
          <Link href="/issues" className="hover:text-gray-700 transition-colors">Issues</Link>
          <MdChevronRight className="text-gray-400" />
          <span className="hover:text-gray-700 cursor-default">{selectedProject.name}</span>
          <MdChevronRight className="text-gray-400" />
          <Link href={`/issues/${issueId}`} className="hover:text-gray-700 transition-colors font-mono text-xs">{issue.issueKey}</Link>
          <MdChevronRight className="text-gray-400" />
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Update the details of the issue</p>
          </div>
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-5">
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-base font-semibold text-gray-900 mb-5">Issue Details</h2>
                ...
                <div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setProjectId(mockProjects[(idx + 1) % mockProjects.length].id);
                    }}
                  >
                    ...
                    <span className="flex-1 text-left text-gray-800 truncate">{selectedProject.name}</span>
                    <MdExpandMore className="text-gray-400 shrink-0" />
                  </button>
                </div>
                ...
                <div>
                  ...
                  <SelectDropdown
                    ...
                    renderOption={(v) => {
                      ...
                      return <><span className={t.color}>{t.icon}</span><span>{t.label}</span></>;
                    }}
                    renderSelected={(v) => {
                      const t = ISSUE_TYPES.find((x) => x.value === v)!;
                      return <span className="flex items-center gap-2"><span className={t.color}>{t.icon}</span>{t.label}</span>;
                    }}
                  />
                </div>
              </div>
              ...
              <div className="mb-5">
                ...
                <div className="border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-400 transition-colors">
                  ...
                  <div className="flex items-center gap-0.5 px-3 py-2 border-b border-gray-100 bg-gray-50 flex-wrap">
                    ...
                    {[MdFormatBold, MdFormatItalic, MdFormatUnderlined, MdStrikethroughS, MdCode].map((Icon, i) => (
                      <button key={i} type="button" className="p-1 rounded text-gray-500 hover:bg-gray-200 hover:text-gray-700 transition-colors">
                        <Icon className="text-base" />
                      </button>
                    ))}
                    <span className="w-px h-4 bg-gray-200 mx-1" />
                    {[MdFormatListBulleted, MdFormatListNumbered, MdFormatAlignLeft, MdFormatAlignCenter].map((Icon, i) => (
                      <button key={i} type="button" className="p-1 rounded text-gray-500 hover:bg-gray-200 hover:text-gray-700 transition-colors">
                        <Icon className="text-base" />
                      </button>
                    ))}
                    <span className="w-px h-4 bg-gray-200 mx-1" />
                    {[MdLink, MdImageIcon, MdFormatQuote, MdMoreHoriz].map((Icon, i) => (
                      <button key={i} type="button" className="p-1 rounded text-gray-500 hover:bg-gray-200 hover:text-gray-700 transition-colors">
                        <Icon className="text-base" />
                      </button>
                    ))}
              ...
              <div className="grid grid-cols-3 gap-4 mb-5">
                <div>
                  ...
                  <SelectDropdown
                    ...
                    renderOption={(v) => {
                      ...
                      return <><span className={p.color}>{p.icon}</span><span>{p.label}</span></>;
                    }}
                    renderSelected={(v) => {
                      const p = PRIORITIES.find((x) => x.value === v)!;
                      return <span className="flex items-center gap-2"><span className={p.color}>{p.icon}</span>{p.label}</span>;
                    }}
                  />
                </div>
                ...
                <div>
                  ...
                  <SelectDropdown
                    ...
                    renderOption={(id) => {
                      ...
                      return (
                        <>
                          <img src={u.imageUrl} alt={u.name} className="w-5 h-5 rounded-full object-cover shrink-0" />
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-700 cursor-not-allowed">
                    <img src={issue.reporter.imageUrl} alt={issue.reporter.name} className="w-5 h-5 rounded-full object-cover shrink-0" />
                    ...
                    <div ref={labelDropRef} className="relative">
                      ...
                      {labelDropOpen && (
                        <div className="absolute z-50 left-0 top-7 w-40 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
                          {ALL_LABELS.filter((l) => !selectedLabels.includes(l.id)).map((label) => (
                            <button
                              key={label.id}
                              type="button"
                              onClick={() => { toggleLabel(label.id); setLabelDropOpen(false); }}
                          ...
                          {ALL_LABELS.filter((l) => !selectedLabels.includes(l.id)).length === 0 && (
                            <p className="px-3 py-2 text-xs text-gray-400">All labels added</p>
                          )}
                        </div>
                      )}
                    </div>
              ...
              <div>
                ...
                {existingFiles.length > 0 && (
                  <div className="flex flex-col gap-2 mb-3">
                    {existingFiles.map((file, i) => (
                      <div key={i} className="flex items-center gap-3 px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg group">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-400">{file.size} &bull; Uploaded on {file.date}</p>
                        </div>
                        <button
                          type="button"
                ...
                {newFiles.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-3">
                    {newFiles.map((name, i) => (
                      <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg text-xs">
                        {name}
                        <button
                          type="button"
                ...
                >
                  <MdOutlineCloudUpload className={`text-3xl ${dragOver ? "text-blue-500" : "text-blue-400"}`} />
                  <p className="text-sm text-gray-600">
                    Drag &amp; drop files here or{" "}
                    <span className="text-blue-600 font-medium hover:underline">click to browse</span>
                  </p>
                  <p className="text-xs text-gray-400">Supports PNG, JPG, GIF, PDF, DOC, ZIP (Max 10MB)</p>
                  <input
                    ref={fileInputRef}
                    type="file"
                    multiple
          ...
          <div className="w-80 shrink-0 flex flex-col gap-5">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <SelectDropdown
                ...
                onChange={(v) => setStatus(v as "BACKLOG" | "TODO" | "IN_PROGRESS" | "IN_REVIEW" | "DONE" | "BLOCKED")}
                renderOption={(v) => {
                  const s = STATUSES.find((x) => x.value === v)!;
                  return <><span className={`w-2 h-2 rounded-full ${s.dot} shrink-0`} /><span>{s.label}</span></>;
                }}
                renderSelected={(v) => {
                  const s = STATUSES.find((x) => x.value === v)!;
                  return (
              ...
              />
              <p className="mt-2 text-xs text-gray-500">Issues are created in &ldquo;To Do&rdquo; status by default.</p>
            </div>

            {/* Additional Fields */}
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <h3 className="text-sm font-semibold text-gray-900 mb-4">Additional Fields</h3>
              <div className="flex flex-col gap-4">
                <SelectDropdown label="Environment" value={environment} placeholder="Select environment" options={ENVIRONMENTS} onChange={setEnvironment} />
                <SelectDropdown label="Version"     value={version}     placeholder="Select version"     options={VERSIONS}      onChange={setVersion} />
                <SelectDropdown label="Component"   value={component}   placeholder="Select component"   options={COMPONENTS}    onChange={setComponent} />
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 flex flex-col gap-3">
              <div>
                ...
                <p className="text-sm text-gray-800 font-medium">{formatDate(issue.createdAt)}</p>
              </div>
              <div className="border-t border-gray-100" />
              <div>
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-0.5">Last Updated</p>
                <p className="text-sm text-gray-800 font-medium">{formatDate(issue.updatedAt)}</p>
              </div>
            </div>
          </div>
        </div>
