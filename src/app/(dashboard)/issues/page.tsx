...
export default function IssuesPage() {
  ...
  const stats = useMemo(() => ({
    ...
    open:       mockIssues.filter((i) => i.status === "TODO" || i.status === "BACKLOG").length,
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <nav className="flex items-center gap-1.5 text-xs text-gray-400 mt-0.5">
            ...
            <span>›</span>
            <Link href="/projects/p1" className="hover:text-blue-500 transition-colors">
              E-commerce Website
            </Link>
            <span>›</span>
      ...
      <div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          { label: "Total Issues", value: stats.total,      icon: <MdAssignment className="text-blue-500 text-xl" />,          iconBg: "bg-blue-50"   },
          { label: "Open",         value: stats.open,       icon: <MdErrorOutline className="text-orange-500 text-xl" />,       iconBg: "bg-orange-50" },
          { label: "In Progress",  value: stats.inProgress, icon: <MdHourglassBottom className="text-violet-500 text-xl" />,    iconBg: "bg-violet-50" },
          { label: "In Review",    value: stats.inReview,   icon: <MdPendingActions className="text-cyan-500 text-xl" />,       iconBg: "bg-cyan-50"   },
          { label: "Done",         value: stats.done,       icon: <MdCheckCircleOutline className="text-emerald-500 text-xl" />,iconBg: "bg-emerald-50"},
          { label: "Overdue",      value: stats.overdue,    icon: <MdWarningAmber className="text-red-500 text-xl" />,          iconBg: "bg-red-50"    },
        ].map((s) => (
          <div key={s.label} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[11px] text-gray-500 leading-none mb-1 truncate">{s.label}</p>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <div className="relative">
            <select className="appearance-none pl-3 pr-8 py-1.5 text-sm border border-gray-200 rounded-lg bg-white text-gray-700 focus:outline-none cursor-pointer">
              ...
              {["frontend", "backend", "bug", "feature", "testing", "security"].map((l) => (
          ...
          <table className="w-full text-sm min-w-[900px]">
            ...
            <tbody className="divide-y divide-gray-100">
              {paginated.length === 0 ? (
                ...
                paginated.map((issue) => (
                  <tr key={issue.id} className="hover:bg-gray-50/70 transition-colors group">
                    {/* Checkbox */}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50/50">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="relative ml-2">
              ...
              >
                {ROWS_OPTIONS.map((n) => (
                  <option key={n} value={n}>Rows per page: {n}</option>
                ))}
              </select>
