...
const GROUPS: NotifGroup[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        desc: "When an issue you follow changes status.",
      },
      {
        key: "due",
        label: "Due Date Reminders",
        desc: "Reminders before an issue is due.",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        desc: "When a new member joins the workspace.",
      },
      {
        key: "project",
        label: "Project Updates",
        desc: "When a project you belong to is updated.",
      },
    ],
  },
];
...
function buildDefault(): NotifState {
  ...
  for (const g of GROUPS)
    for (const i of g.items) s[i.key] = { email: true, inapp: true };
  return s;
}
        ...
        {GROUPS.map((group) => (
          <div key={group.label}>
            ...
            {group.items.map((item) => (
              <div
                key={item.key}
                className="grid grid-cols-[1fr_100px_100px] gap-4 items-center px-6 py-3.5 border-b border-gray-50 last:border-0"
              >
                <div>
                ...
                <div className="flex justify-center">
                  <Toggle
                    ...
                    onChange={() => toggle(item.key, "inapp")}
                  />
                </div>
              </div>
            ))}
